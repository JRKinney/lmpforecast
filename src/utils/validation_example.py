...
def validate_data_example():
    ...
    try:
        ...
        validated_price_data = PriceDataSchema.validate(price_data)
        print("✅ Price data validation successful!")
        
        # Validate weather data against schema
        validated_weather_data = WeatherDataSchema.validate(weather_data)
    ...
    try:
        ...
        validate_dataframe(invalid_forecast, PriceForecastSchema)
        print("✅ Validation passed (this shouldn't happen!)")
    except Exception as e:
        print(f"❌ Validation failed as expected due to bounds violation")
