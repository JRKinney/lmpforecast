...
def align_time_series(
    ...
    if 'price_node' in aligned_price.columns and aligned_price['price_node'].isna().any():
        aligned_price['price_node'] = aligned_price['price_node'].fillna(price_data['price_node'].iloc[0])
    
    if 'location' in aligned_weather.columns and aligned_weather['location'].isna().any():
        aligned_weather['location'] = aligned_weather['location'].fillna(weather_data['location'].iloc[0])
...
def create_time_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    result['hour'] = result.index.hour / 23.0  # Scaled to [0,1]
    ...
    result['is_business_hour'] = (result.index.hour >= 8) & (result.index.hour < 18) & (~result['is_weekend'])
    result['is_peak_hour'] = (result.index.hour >= 14) & (result.index.hour < 20)
...
def create_rolling_features(
    ...
    for window in windows:
        for func_name, func in functions.items():
            result[f'{column}_{func_name}_{window}h'] = result[column].rolling(window=window, min_periods=1).apply(func)
...
def prepare_data_for_model(
    ...
    # Lag features
    if include_lags:
        merged_data = create_lag_features(merged_data, 'price', lag_hours=[1, 24, 48, 168])
        
        # Also create lags for important weather variables
        for weather_var in ['temperature', 'solar_irradiance']:
            merged_data = create_lag_features(merged_data, weather_var, lag_hours=[1, 24])
    ...
    if scale_features:
        ...
        numeric_columns = merged_data.select_dtypes(include=['float', 'int']).columns.tolist()
        if 'price' in numeric_columns:
            numeric_columns.remove('price')
        
        # Scale features
        feature_scaler = MinMaxScaler(feature_range=(-1, 1))
        merged_data[numeric_columns] = feature_scaler.fit_transform(merged_data[numeric_columns])
    ...
    feature_columns = merged_data.select_dtypes(include=['float', 'int']).columns.tolist()
    ...
    for i in range(len(merged_data) - seq_length - forecast_horizon + 1):
        ...
        y_seq = merged_data.iloc[i+seq_length:i+seq_length+forecast_horizon]['price'].values
    ...
    result = {
        ...
        'timestamps': {
            'sequence_start_dates': merged_data.index[:-seq_length-forecast_horizon+1].tolist(),
            'forecast_start_dates': merged_data.index[seq_length:-forecast_horizon+1].tolist()
        }
    }
...
def preprocess_data(
    ...
    if scale:
        ...
        if weather_data is not None:
            weather_cols = [col for col in data.columns if col not in ['price', 'hour_sin', 'hour_cos', 'dow_sin', 'dow_cos']]
            if weather_cols:
