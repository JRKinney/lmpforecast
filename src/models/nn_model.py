...
class NNModel:
    ...
    def _create_sequences(
        ...
        for i in range(len(data) - seq_length - forecast_horizon + 1):
            ...
            y.append(data[i + seq_length:i + seq_length + forecast_horizon, 0])  # Only price column for target
    ...
    def predict(
        ...
        if weather_data is not None:
            weather_cols = [col for col in data.columns if col != 'price']
            if weather_cols:
                data[weather_cols] = self.weather_scaler.transform(data[weather_cols].values)
        
        # Ensure we have at least seq_length points
        if len(data) < self.seq_length:
            raise ValueError(f"Input data must have at least {self.seq_length} time points")
