...
class GARCHModel:
    ...
    def predict(
        ...
        forecast = self.result.forecast(
            horizon=forecast_horizon, reindex=False, x=X_pred
        )
        ...
        last_date = self.result.data.index[-1]
        forecast_index = pd.date_range(
            start=last_date + pd.Timedelta(hours=1), periods=forecast_horizon, freq="H"
        )

        # Create forecast DataFrame
        forecast_df = pd.DataFrame(
            variance, index=forecast_index, columns=["variance_forecast"]
        )
    ...
    def calculate_confidence_intervals(
        ...
        variance_forecast: DataFrame[ForecastSchema],
        ...
        volatility_values = variance_forecast.loc[
            common_index, "volatility_forecast"
        ].values
