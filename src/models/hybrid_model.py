...
class HybridModel:
    ...
    def __init__(
        ...
        logger.info(f"Initialized HybridModel with seq_length={seq_length}, forecast_horizon={forecast_horizon}")
    ...
    def _preprocess_data(
        ...
        merged_data['hour'] = merged_data.index.hour / 23.0  # Scale to [0, 1]
        ...
        X, y = self._create_sequences(merged_data, feature_columns)
    ...
    def _create_sequences(
        ...
        for i in range(n_samples):
            ...
            y_sequence = data['price'].iloc[i+self.seq_length:i+self.seq_length+self.forecast_horizon].values
    ...
    def predict(
        ...
        if len(price_data) < self.seq_length:
            raise ValueError(f"Not enough historical price data. Need at least {self.seq_length} points.")
        ...
        # Create time features
        merged_data['hour'] = merged_data.index.hour / 23.0
        merged_data['day_of_week'] = merged_data.index.dayofweek
        ...
        input_sequence = merged_data[self.feature_columns].iloc[-self.seq_length:].values
        ...
        garch_forecast = self.garch_model.forecast(horizon=self.forecast_horizon, reindex=False)
        ...
        try:
            validated_forecast = PriceForecastSchema.validate(forecast_df)
