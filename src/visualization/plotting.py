...
from src.utils.schemas import PriceDataSchema, WeatherDataSchema, PriceForecastSchema


def plot_price_forecast(
    forecasts: DataFrame[PriceForecastSchema],
...
def plot_price_components(
    ...
    price_subset = price_data[(price_data.index >= start_date) & (price_data.index <= end_date)]
    weather_subset = weather_data[(weather_data.index >= start_date) & (weather_data.index <= end_date)]
    ...
    hourly_price = merged_data.groupby(merged_data.index.hour)['price'].mean()
    fig.add_trace(
        go.Bar(
    ...
    daily_price = merged_data.groupby(merged_data.index.dayofweek)['price'].mean()
...
def plot_model_performance(
    actual_prices: Union[pd.Series, List[float]],
    forecasted_prices: Union[pd.Series, List[float]],
    ...
    Args:
        actual_prices: Series or list of actual price values
        forecasted_prices: Series or list of forecasted price values
    ...
    if isinstance(actual_prices, list):
        if isinstance(forecasted_prices[0], pd.Series):
            dates = forecasted_prices.index
        else:
            dates = pd.date_range(start='2023-01-01', periods=len(actual_prices), freq='H')
        actual_prices = pd.Series(actual_prices, index=dates)
        forecasted_prices = pd.Series(forecasted_prices, index=dates)
    ...
    fig.add_trace(
        go.Table(
            ...
            cells=dict(
                values=[
                    ["MAE", "MAPE", "RMSE", "R²"],
                    [f"${mae:.2f}/MWh", f"{mape:.2f}%", f"${rmse:.2f}/MWh", f"{r2:.4f}"]
                ],
                fill_color='lavender',
                align='center'
            )
