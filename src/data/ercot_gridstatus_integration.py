...
from src.utils.schemas import PriceDataSchema, WeatherDataSchema, validate_dataframe
...
class ErcotGridstatusIntegration:
    ...
    def fetch_price_data(
        ...
        try:
            ...
            price_data = pd.DataFrame({
                'price': raw_price_data['price'].values,
            ...
            raise ValueError(f"Failed to fetch ERCOT price data: {str(e)}")
    ...
    def fetch_weather_data(
        ...
        if location not in self.location_to_weather_zone:
            ...
            raise ValueError(f"Invalid location: '{location}'. Valid options are: {valid_locations}")
        ...
        try:
            ...
            # Filter for the specific weather zone
            if weather_zone in temp_data.columns:
                temperature = temp_data[weather_zone]
            else:
                # If exact zone not found, use average as fallback
                temperature = temp_data.mean(axis=1)
            ...
            try:
                ...
                if 'Wind Speed' in wind_data.columns:
                    ...
                    wind_speed = np.sqrt(wind_data['Wind Generation'] / 100)  # Simplified conversion
                else:
                    # Generate synthetic wind data if not available
                    date_range = pd.date_range(start=start_date, end=end_date, freq='H')
                    wind_speed = pd.Series(
                        5 + 3 * np.sin(np.pi * date_range.hour / 12) + np.random.exponential(2, size=len(date_range)),
                        index=date_range
                    )
            except Exception:
            ...
            try:
                ...
                if 'Solar Irradiance' in solar_data.columns:
                    ...
                    solar_irradiance = solar_data['Solar Generation'] * 2  # Simplified conversion
                else:
                    ...
                    solar_peak = np.sin(np.pi * (hour_of_day - 6) / 12) * solar_hours  # Peak at noon
                    seasonal_factor = 0.7 + 0.3 * np.sin(2 * np.pi * (day_of_year - 15) / 365)  # Stronger in summer
                    solar_irradiance = pd.Series(
                        1000 * solar_peak * seasonal_factor + np.random.normal(0, 50, size=len(date_range)),
                        index=date_range
                    )
                    solar_irradiance = solar_irradiance.clip(lower=0)  # No negative irradiance
            except Exception:
            ...
            if isinstance(temperature, pd.Series) and len(temperature) > 0:
                ...
                humidity = base_humidity - 0.5 * (temperature - base_temp) + np.random.normal(0, 10, size=len(temperature))
            ...
            if isinstance(temperature, pd.Series):
                ...
                temperature = pd.Series(np.random.normal(25, 5, size=len(common_index)), index=common_index)
            ...
            # Validate against schema
            try:
                validated_data = validate_dataframe(weather_data, WeatherDataSchema)
                return validated_data
            except Exception as e:
                raise ValueError(f"Weather data validation failed: {str(e)}")
                
        except Exception as e:
            raise ValueError(f"Failed to fetch ERCOT weather data: {str(e)}")
    ...
    def get_available_locations(self) -> Dict[str, str]:
        """
        Get a dictionary of available locations with their corresponding weather zones.
        
        Returns:
            Dictionary mapping location names to ERCOT weather zones
        """
        return {
            location: zone for location, zone in self.location_to_weather_zone.items()
        }
    ...
    def fetch_system_wide_data(
        ...
        """
        try:
            # Fetch system-wide actual load
            system_data = self.ercot.get_system_wide_actual_load(
                date=start_date,
                end=end_date,
                verbose=False
            )
            ...
            raise ValueError(f"Failed to fetch system-wide data: {str(e)}")
    ...
    def fetch_fuel_mix(
        ...
        try:
            ...
            if all_data:
                ...
                raise ValueError("No fuel mix data available for the specified date range")
        except Exception as e:
            raise ValueError(f"Failed to fetch fuel mix data: {str(e)}") 