...
from src.utils.schemas import WeatherDataSchema, validate_dataframe
...
class ErcotWeatherData:
    ...
    def __init__(self, data_dir: Optional[str] = None):
        ...
        """
        self.data_dir = data_dir or os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'data', 'weather')
    ...
    def load_data(
        ...
        humidity = base_humidity - 0.5 * (temperature - base_temp) + np.random.normal(0, 10, size=len(date_range))
        humidity = np.clip(humidity, 0, 100)  # Ensure between 0-100%
        
        # Wind speed: some daily pattern with randomness
        wind_speed = 5 + 3 * np.sin(np.pi * hour_of_day / 12) + np.random.exponential(2, size=len(date_range))
        ...
        solar_irradiance = 1000 * solar_peak * seasonal_factor + np.random.normal(0, 50, size=len(date_range))
    ...
    def clean_outliers(
        ...
        for column in numeric_columns:
            if method == 'iqr':
                ...
                cleaned_data.loc[
                    (cleaned_data[column] - mean).abs() > threshold * std, 
                    column
        ...
        cleaned_data['solar_irradiance'] = cleaned_data['solar_irradiance'].clip(lower=0)
        
        # Validate after cleaning
        return validate_dataframe(cleaned_data, WeatherDataSchema)
    
    def calculate_summary_statistics(self, data: DataFrame[WeatherDataSchema]) -> Dict[str, Dict[str, Any]]:
        """
        ...
        Returns:
            Dictionary of summary statistics by variable
        """
    ...
    def get_extreme_weather_events(
        ...
        return df[df['any_extreme_condition']].copy() 