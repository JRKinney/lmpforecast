...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run ERCOT price forecasting with real data from gridstatus"
    )
    ...
    # Output options
    parser.add_argument(
        "--save-plot", action="store_true", help="Save forecast plot to file"
    )
    parser.add_argument(
        "--save-model", action="store_true", help="Save trained model to file"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
...
def main() -> int:
    ...
    logger.info(
        f"Setting up forecast for {args.price_node} with data from {start_date} to {end_date}"
    )
    ...
    try:
        ...
        if args.use_synthetic:
            ...
            try:
                ...
                logger.info(
                    f"Successfully fetched real price data: {len(price_data)} hours"
                )
                ...
                logger.info(
                    f"Successfully fetched real weather data: {len(weather_data)} hours"
                )
        ...
        recent_price = price_data[
            price_data.index > (train_end - timedelta(hours=args.seq_length))
        ]
        recent_weather = weather_data[
            weather_data.index > (train_end - timedelta(hours=args.seq_length))
        ]
        ...
        logger.info(
            f"Average forecasted price: ${forecast['price_forecast'].mean():.2f} per MWh"
        )
        logger.info(
            f"Price range: ${forecast['price_forecast'].min():.2f} to ${forecast['price_forecast'].max():.2f} per MWh"
        )
