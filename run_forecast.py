...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--save-plot", action="store_true", help="Save the forecast plot as HTML file"
    )
...
def main() -> None:
    ...
    price_data: DataFrame[PriceDataSchema] = price_loader.load_data(
        start_date=args.start_date, end_date=args.end_date, price_node=args.price_node
    )

    print(f"Loading weather data for location {args.location}...")
    weather_data: DataFrame[WeatherDataSchema] = weather_loader.load_data(
        start_date=args.start_date, end_date=args.end_date, location=args.location
    )
